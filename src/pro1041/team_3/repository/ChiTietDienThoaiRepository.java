...
public class ChiTietDienThoaiRepository extends Repository<ChiTietDienThoai, UUID, ChiTietDienThoaiResponse>{
    ...
    public ChiTietDienThoaiRepository() {
        ...
        this.resCon = "new fpoly.pro1041.nhom_3.dto.ChiTietDienThoaiResponse (a.id, a.ma, a.mauSac.ten, a.dienThoai.ten, a.hang.ten, a.tinhTrang, "
                + "a.giaBan, a.trangThai, a.hinhAnh, a.imei, a.ram, a.boNho, a.moTa)";
    }
    ...
    public List<ChiTietDienThoaiResponse> getAllByTinhTrang1() {
        try {
            List<ChiTietDienThoaiResponse> lst;
            session = HibernateUtil.getSession();
            String hql = "SELECT " + resCon + " FROM " + className + " a WHERE a.tinhTrang = 1 ";
            Query query = session.createQuery(hql);
            lst = query.getResultList();
            return lst;
        } catch (Exception e) {
    ...
    public List<ChiTietDienThoaiResponse> getAllByTinhTrang0() {
        try {
            List<ChiTietDienThoaiResponse> lst;
            session = HibernateUtil.getSession();
            String hql = "SELECT " + resCon + " FROM " + className + " a WHERE a.tinhTrang = 0 ";
            Query query = session.createQuery(hql);
            lst = query.getResultList();
            return lst;
        } catch (Exception e) {
